...
void menu(struct singer *people) // 接入节点参数
{
    ...
    case '1':
        ...
        else { creatList(people); printf("\n");}
        getchar(); system("clear");
        break;

    case '2':
        inputScore(people); printf("\n"); system("clear");
        break;

    case '3':
        scoreSort(people); getchar(); printf("\n"); system("clear");
        break;

    case '4':
        search(people); getchar(); printf("\n"); system("clear");
        break;

    case '5':
        attend(people); getchar(); printf("\n"); system("clear");
        break;

    case '6':
        writeFile(people); getchar(); printf("\n"); system("clear");
        break;

    case '7':
        printf("         您是否要退出系统?(Y/N):"); getchar(); choise = getchar();
...
void creatList(struct singer *people)
{
    ...
    for (int i = 0; i < n; i++)
    {
        printf("         请输入选手%d的编号:", i + 1); scanf("%d", &people[i].Number);
        printf("         请输入选手%d的姓名:", i + 1); scanf("%s", people[i].Name);
    }
    flag = 1; fgets(x, 2, stdin);
...
void inputScore(struct singer *people)
{
    ...
    printf("\n         选手成绩已输入完成，请按回车键确认并返回主菜单"); getchar();
}
...
void scoreSort(struct singer *people)
{
    for (int i = 1; i < n; i++)
        for (int j = 0; j < n - i; j++)
            if (people[j].scoreAve < people[j + 1].scoreAve) { struct singer temp = people[j + 1]; people[j + 1] = people[j]; people[j] = temp;}
    printf("         %-10s %-20s %-10s %-10s\n", "编号", "姓名", "总成绩", "平均成绩");
    for (int i = 0; i < n; i++)
        printf("         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve);
    printf("\n         选手平均成绩已排序完成，请按回车键确认并返回主菜单"); getchar();
}
...
void search(struct singer *people)
{
    ...
    case '1':
        ...
        printf("         %-10s %-20s %-10s %-10s\n", "编号", "姓名", "总成绩", "平均成绩");
        for (int i = 0; i < n; i++)
            if (people[i].Number == num1) { printf("         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve); count++;}
            else if (i == n - 1 && !count) printf("         不存在对应选手！\n");
        if (count) printf("         查询完毕，共有%d名符合条件的选手,请按回车键确认并返回主菜单", count);
        break;
    ...
    case '2':
        ...
        printf("         %-10s %-20s %-10s %-10s\n", "编号", "姓名", "总成绩", "平均成绩");
        for (int i = 0; i < n; i++)
            if (!strcmp(people[i].Name, name)){ printf("         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve); count++;}
            else if (i == n - 1 && !count) printf("         不存在对应选手！\n");
        if (count) printf("         查询完毕，共有%d名符合条件的选手,请按回车键确认并返回主菜单", count);
        break;
    ...
    case '3':
        ...
        printf("         %-10s %-20s %-10s %-10s\n", "编号", "姓名", "总成绩", "平均成绩");
        for (int i = 0; i < n; i++)
            if (people[i].scoreSum == score) { printf("         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve); count++;}
            else if (i == n - 1 && !count) printf("         不存在对应选手！\n");
        if (count) printf("         查询完毕，共有%d名符合条件的选手,请按回车键确认并返回主菜单", count);
        break;
    ...
    case '4':
        ...
        printf("         %-10s %-20s %-10s %-10s\n", "编号", "姓名", "总成绩", "平均成绩");
        for (int i = 0; i < n; i++)
            if ((int)people[i].scoreAve == (int)ave) { printf("         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve); count++;}
            else if (i == n - 1 && !count) printf("         不存在对应选手！\n");
        if (count) printf("         查询完毕，共有%d名符合条件的选手,请按回车键确认并返回主菜单", count);
        break;
...
void writeFile(struct singer *people)
{
    ...
    fprintf(fp, "         ---------------------------------------------------------------------------------\n");
    for (int i = 0; i < n; i++) fprintf(fp, "         %-8d %-18s %-7d %-8.2lf\n", people[i].Number, people[i].Name, people[i].scoreSum, people[i].scoreAve);
    fclose(fp);
    fp = NULL;
